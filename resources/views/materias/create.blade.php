...
<div class="container-fluid">
    <x-ui.page-header title="Nueva Materia" subtitle="Registrar una materia" :actions="[['label' => 'Volver', 'route' => route('materias.index')]]" />
    <x-ui.card>
        <form action="{{ route('materias.store') }}" method="POST">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select name="escuela_id" class="form-control @error('escuela_id') is-invalid @enderror" required>
                        ...
                        @foreach($escuelas as $escuela)
                            <option value="{{ $escuela->id }}" {{ old('escuela_id') == $escuela->id ? 'selected' : '' }}>{{ $escuela->nombre }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="clave" class="form-control @error('clave') is-invalid @enderror" value="{{ old('clave') }}" required>
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="horas_semana" class="form-control @error('horas_semana') is-invalid @enderror" value="{{ old('horas_semana') }}" min="0">
                    @error('horas_semana')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-4">
                    <label for="creditos">Créditos</label>
                    <input type="number" name="creditos" class="form-control @error('creditos') is-invalid @enderror" value="{{ old('creditos') }}" min="0">
                    @error('creditos')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-4">
                    <div class="form-check mt-4">
                        <input type="checkbox" name="activa" value="1" {{ old('activa', true) ? 'checked' : '' }} class="form-check-input">
