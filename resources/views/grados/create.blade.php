...
<div class="container-fluid">
    <x-ui.page-header title="Nuevo Grado" subtitle="Registrar un grado académico" :actions="[['label' => 'Volver', 'route' => route('grados.index')]]" />
    <x-ui.card>
                ...
                <div class="col-md-6">
                    ...
                    <select name="nivel_educativo_id" class="form-control @error('nivel_educativo_id') is-invalid @enderror" required>
                        ...
                        @foreach($niveles as $nivel)
                            <option value="{{ $nivel->id }}" {{ old('nivel_educativo_id') == $nivel->id ? 'selected' : '' }}>{{ $nivel->nombre }}</option>
                        @endforeach
                    </select>
